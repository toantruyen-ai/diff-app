...
- **Trích xuất Selector & Filter Pod Cô lập**: Đối với các workload (`Deployment`, `StatefulSet`, `DaemonSet`, `ReplicaSet`, `Job`), `projectRow()` trong `resourceFormatter.js` lưu giữ `matchLabels` (từ `spec.selector.matchLabels`). Nếu thiếu, `resolveMatchLabels()` tự động tra cứu K8s API (`AppsV1Api`/`BatchV1Api`) để lấy `matchLabels`. Service áp dụng `labelSelector` khi gọi `listNamespacedPod()` và lọc qua `filterPodsForWorkload()` (đối chiếu label, ownerReferences hoặc tiền tố tên), đảm bảo cô lập 100% log của workload mục tiêu, không bị lọt log của app khác trong cùng namespace.
...
- **Follow / tail mode**: auto-scroll đáy; **tự động uncheck checkbox Follow (`#mpl-follow-chk`) khi user cuộn lên (`!isAtBottom`)**; tự động check và bật lại khi cuộn về sát đáy (`isAtBottom`, ngưỡng 15px chống lệch do DPI/zoom). Reset về `true` khi khởi tạo session mới hoặc clear logs. Cắt đầu *trong lúc* xem lịch sử → neo scroll theo **seq** (không pixel tuyệt đối) để màn hình không nhảy.
...
## 11. Cập nhật tinh chỉnh thực tế (Implementation Notes)

- **Cô lập Log theo Workload (`src/main/services/multiPodLogService.js`, `src/main/utils/resourceFormatter.js`, `renderer/app.js`)**:
  - `projectRow()` bổ sung `matchLabels: spec.selector?.matchLabels` cho các workload (`deployments`, `statefulsets`, `daemonsets`, `replicasets`, `jobs`).
  - `startManageLogs()` ở renderer truyền `matchLabels` vào payload `workload`.
  - `multiPodLogService.js` triển khai `resolveMatchLabels()` để tra cứu dynamic nếu payload chưa có, và `filterPodsForWorkload()` để lọc danh sách pod theo label matching, `ownerReferences` hoặc tiền tố tên. Đảm bảo 100% không rò rỉ log của ứng dụng khác trong namespace.
- **Follow Mode Scroll Auto-Uncheck (`src/renderer/utils/multiPodLogViewer.js`)**:
  - Lắng nghe sự kiện `scroll` trên viewport `#mpl-output-viewport`.
  - Cuộn lên xa đáy (`!isAtBottom`) → Tự động cập nhật `isFollowMode = false` và uncheck checkbox `followChk.checked = false`.
  - Cuộn về sát đáy (`isAtBottom`, ngưỡng 15px) → Tự động bật `isFollowMode = true` và check lại `followChk.checked = true`.
  - Đặt lại `isFollowMode = true` và `followChk.checked = true` khi bắt đầu session mới hoặc bấm Clear Log.
