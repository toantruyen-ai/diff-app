...
Giao diện Cài đặt ứng dụng (`#manage-settings-pane`) tách riêng phần **AI Troubleshooting & Diagnostics** thành một Section độc lập (id `manage-settings-section-ai`), thay vì lẫn vào cấu hình chung. Section này cho phép chọn **AI CLI Provider** (`claude` hoặc `agy`), **test kết nối CLI** qua hàm `testCli` (chạy thử binary đã resolve, báo phiên bản/trạng thái đăng nhập), và bổ sung card cấu hình **Grafana & Observability Datasources (`k8senvdiff_ai_config`)**.

Các tham số cấu hình Observability được nhập và lưu trữ tập trung trong **Audit Database (Azure SQL)** theo `clusterId` bao gồm:
- **Grafana Base URL** (`grafana_url`): URL kết nối đến Grafana dashboard (ví dụ: `https://grafana.example.com`).
- **Service Account Bearer Token** (`service_account_token`): Token xác thực Service Account kết nối Grafana API.
- **Loki Datasource Name / UID** (`loki_datasource`): Tên hoặc UID của Loki datasource tra cứu logs.
- **Mimir / Prometheus Datasource Name / UID** (`mimir_datasource`): Tên hoặc UID của Mimir / Prometheus datasource tra cứu metrics.
- **Tempo Datasource Name / UID** (`tempo_datasource`): Tên hoặc UID của Tempo datasource tra cứu distributed traces.

Hàm `getAiConfig(ref, contextName)` và `saveAiConfig(ref, contextName, config)` trong `auditDb.js` tự động load/save các giá trị này tương ứng với Cluster đang chọn mỗi khi người dùng đổi context hoặc lưu cài đặt.

### 12.2. Tải bản phân tích lưu trữ theo Pod Name từ Audit Database & Cách ly state Drawer (`initPodAnalysis`)

Khi người dùng chọn một Pod và bấm sang tab **AI Analyze**:
1. **Cách ly state giữa các Pod (`openManageDrawer` / `closeManageDrawer`)**: Xóa sạch toàn bộ nội dung HTML và trạng thái cũ trong `manageAnalyzeContent` để tránh việc hiển thị nhầm báo cáo của Pod đã xem trước đó.
2. **Auto-load bản phân tích lưu sẵn theo `(namespace, pod_name)`**: Hàm `initPodAnalysis()` truy vấn lịch sử trong **Audit Database (Azure SQL `k8senvdiff_ai_analysis`)** qua `auditDb.getAnalysisHistory`. Nếu tìm thấy bản ghi phân tích cũ của Pod đó, ứng dụng hiển thị tức thì (instant load) báo cáo gần nhất cùng mốc thời gian lưu trữ (`Loaded saved analysis (<timestamp>)`) kèm bảng lịch sử phân tích bên dưới.
3. **Định danh Cluster bất biến toàn cục (`resolveClusterId`)**: Hàm `resolveClusterId(ref, contextName)` được chuẩn hóa thành Global Helper duy nhất tại `src/main/utils/k8sHelper.js`, bóc tách URL API Server (`cluster.server`) duy nhất của Kubernetes Cluster để tạo mã `cluster_id`. Được dùng chung nhất quán cho toàn bộ các tính năng ứng dụng: **YAML Version Management (Audit Trail/Revisions)**, **Cluster Events DB**, và **AI Troubleshooting DB**, bảo đảm tất cả lịch sử chỉnh sửa YAML và báo cáo chẩn đoán AI không bao giờ bị trôi hay mất khi chuyển đổi qua lại giữa các cluster.
