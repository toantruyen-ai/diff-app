...
4. Apply the minimal fix at the root cause — no scope creep, keep audit/write-gate & teardown intact.
5. Run full `npm test` (100% pass); `node --check` changed files; `npm run build:renderer` if renderer changed.
